...
from services.ga_service import crawl_keyword_products
...
def _run_keyword_search(keyword_id: int, keyword_name: str, task_id: str) -> None:
    ...
    新流程 (纯xdotool物理操作, 无CDP):
    1. xdotool 导航淘宝搜索页 + 输入关键词
    2. DTS 店透视市场分析 → 导出 Excel
    3. 解析 Excel → 入库 PostgreSQL
    """
    logger.info("开始搜索关键词: id=%d name=%s", keyword_id, keyword_name)
    try:
        # ── 统一流程: xdotool搜索 + DTS导出 + 解析入库 ──
        count = crawl_keyword_products(keyword_name)
        logger.info("关键词搜索入库: %d 个商品", count)
        ...
        with _search_lock:
            if count > 0:
                _search_tasks[task_id] = {
                    ...
                    "result": None,
                    "message": f"搜索完成：DTS导出并入库 {count} 个商品",
                    "product_count": count,
                ...
                _search_tasks[task_id] = {
                    ...
                    "keyword_id": keyword_id,
                    "keyword_name": keyword_name,
                    "message": "搜索失败：未能获取商品数据。请确认Chrome已打开、淘宝已登录、DTS扩展已安装。",
                    "started_at": _search_tasks.get(task_id, {}).get("started_at", 0),
                    "finished_at": time.time(),
                }
