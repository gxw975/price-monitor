  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {tab === 'products' && (
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 8, flexWrap: 'wrap', alignItems: 'center', padding: 8, background: '#f9fafb', borderRadius: 6, border: '1px solid #e5e7eb' }}>
            ...
            <button onClick={() => setCatFilter(null)} style={{ padding:'2px 8px',fontSize:11,borderRadius:4,border:'1px solid #d9d9d9',background:catFilter===null?'#1677ff':'#fff',color:catFilter===null?'#fff':'#666',cursor:'pointer' }}>全部({totalProductCount})</button>
      ...
      {hoverTitlePid && (() => { const hp = products.find((pr:any) => pr.product_id === hoverTitlePid); if (!hp) return null;
        return <div style={{position:'fixed',left:Math.max(10, hoverTitlePos.x - 250),top:Math.min(hoverTitlePos.y, window.innerHeight-310),zIndex:9999,pointerEvents:'none',background:'#fff',borderRadius:8,boxShadow:'0 4px 20px rgba(0,0,0,0.2)',padding:12,width:280}}>
