...
var flow_path = '/home/lab-admin/price-monitor/scripts';
...
chrome_step('Page.navigate',{url: 'https://www.taobao.com'}); sleep(1000);
techo('https://www.taobao.com' + ' - ' + chrome.getTitle() + '\n');});

casper.then(function() {techo('wait 3');});
casper.then(function() {casper.wait(3000, function() {});});

casper.then(function() {{techo('click q');
casper.waitFor(function check() {return check_tx('q');},
function then() {chrome.click(tx('q'));},
function timeout() {chrome.capture('/home/lab-admin/price-monitor/scripts/taobao_search_error.png');
this.echo('ERROR - cannot find q').exit(); this.wait(0);});}});

casper.then(function() {{techo('type q as 一米八八奶粉');
casper.waitFor(function check() {return check_tx('q');},
function then() {chrome.sendKeys(tx('q'),'一米八八奶粉');},
function timeout() {chrome.capture('/home/lab-admin/price-monitor/scripts/taobao_search_error.png');
this.echo('ERROR - cannot find q').exit(); this.wait(0);});}});
...
casper.then(function() {{techo('keyboard [enter]'); var fs = require('fs');
if (!sikuli_step('keyboard [enter]')) if (!fs.exists('[enter]') && !false)
{this.echo('ERROR - cannot find image file [enter]').exit(); this.wait(0);} else
{sikuli_step('snap page.png to /home/lab-admin/price-monitor/scripts/taobao_search_error.png');
this.echo('ERROR - cannot find [enter] on screen').exit(); this.wait(0);}}});

casper.then(function() {techo('wait 5');});
casper.then(function() {casper.wait(5000, function() {});});

casper.then(function() {this.echo('SEARCH_COMPLETED');});
